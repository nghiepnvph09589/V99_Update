...
export class DrawPointsScreen extends Component {
    ...
    renderBankSelect = () => {
        ...
        return bankState.map((item, index) => {
            ...
            return <TouchableOpacity
                onPress={() => {
                    if (!isSelected)
                        this.onSelectBank(index, item.id)
                ...
                <Block row center>
                    ...
                    >
                        {item.shortName + " - ******" 
                        + item.codeBankAccount.substring(item.codeBankAccount.length - 4, item.codeBankAccount.length)}
                    </Text>

                </Block>
                {isSelected && <FstImage source={R.images.ic_tick_bank} style={{
                    width: width * 0.05,
                    aspectRatio: 1
                }} resizeMode='contain' />}
        ...
        return (
            <View style={{
                ...
                <Text style={{
                    ...
                    color: theme.colors.black1,
                    marginVertical: 10
                }}>Vui lòng chờ từ 1-3 ngày làm việc để yêu cầu rút điểm của bạn được xác nhận và số tiền nhận được đến tài khoản</Text>
    ...
    drawPointsPress = async () => {
        ...
        try {
            ...
            NavigationUtil.navigate(SCREEN_ROUTER.DETAIL_DRAW_POINTS, { id: res.data.id })
            ...
            <Block>
                <DCHeader isWhiteBackground
                    ...
                    rightComponent={<TouchableOpacity
                        onPress={() => NavigationUtil.navigate(SCREEN_ROUTER.HISTORY_DRAW_POINTS)}>
...
const mapDispatchToProps = {
    updateUserLocal, getWalletPoints, getBank
}

export default connect(mapStateToProps, mapDispatchToProps)(DrawPointsScreen)
