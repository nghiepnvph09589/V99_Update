...
import {
    Block, PrimaryButton,
    LoadingProgress, NumberFormat, FastImage, FstImage
} from '../../components'
import * as theme from '../../constants/Theme'
import I18n from '../../i18n/i18n'
import { addToCart, getUserInfo, getCountCart } from '../../redux/actions'
import NavigationUtil from '../../navigation/NavigationUtil'
import { SCREEN_ROUTER, REDUCER, USER_ACTIVATED } from '../../constants/Constant'
...
export class DetailProductScreen extends Component {
    ...
    checkToken = async () => {
        ...
        if (ObjectUtil.isEmpty(userInfoState.data) && !!token) this.props.getUserInfo()
        this.setState({ token })
    }
        ...
        return (
            <Block>
                <View style={{ paddingHorizontal: '2.5%' }}>
                    <Text style={[theme.fonts.semibold14, {
                        marginTop: 10,
                    }]}>{item.name}</Text>
                    <View style={{
                        flexDirection: "row",
                        ...
                        <NumberFormat
                            ...
                            perfix='đ' />
                        <Text style={{
                            color: item.stockStatus == 1 ? '#00C48C' : theme.colors.red_money,
    ...
    handleBuyNow = () => {
        ...
        callAPI({
            ...
            payload: [itemID],
            onSuccess: res => {
                NavigationUtil.navigate(SCREEN_ROUTER.CART, { listItemIDSelected: [itemID] })
            },
            onError: err => {
                console.log(err);
            },
                ...
                <SafeAreaView style={theme.styles.container}>
                    ...
                    <Block flex={false} row paddingHorizontal={10} paddingVertical={10}>
                        ...
                        <PrimaryButton
                            ...
                            style={styles.button} title={I18n.t('buy_now')} />
                    </Block>
                </SafeAreaView>
            </Block>
        )
